...
def embed_and_store_text(text, filename, owner):
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=0)
    texts = text_splitter.split_text(text)
    documents = [Document(page_content=chunk, metadata={"filename": filename, "owner": owner}) for chunk in texts]
    vectorstore.add_documents(documents)
    st.write(f"Document {filename} stored with {len(documents)} chunks.")
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        embed_and_store_text(text, uploaded_file.name, username)
